...
#define CMD_ADDR  _SFR_IO8(0x4000)
#define DATA_ADDR _SFR_IO8(0x4100)

/**
 * @brief Write a single command to the display.
 */
static inline void write_cmd(uint8_t cmd) { CMD_ADDR = cmd; }

/**
 * @brief Write a single byte of data to the display.
 */
static inline void write_data(uint8_t data) { DATA_ADDR = data; }

/**
 * @brief Write a command and data byte to the display.
 */
static inline void write_cmd_data(uint8_t cmd, uint8_t data) {
    write_cmd(cmd);
    write_data(data);
}

/**
 * @brief Write a 16-bit data word to the display.
 * The high byte is written first, then the low byte.
 */
